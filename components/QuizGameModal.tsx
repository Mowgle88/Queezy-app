...
export default function QuizGameModal(props: QuizGameModalProps) {
  const { visible, onPass, index, numberOfQuizzes, question, correctAnswer, selectedAnswer } = props;
  ...
  return (
    <Modal animationType="slide" transparent={true} visible={visible}>
      <View style={styles.centeredView}>
        <View>
          <Text style={styles.title}>QUESTION {index} OF {numberOfQuizzes}</Text>
          <Text style={styles.question}>{question}</Text>
          <Text style={styles.title}>SELECTED ANSWER</Text>
          <View style={[
            styles.answerContainer,
            trueAnswer ? styles.correctAswerContainer : styles.wrongAswerContainer
          ]}>
            <Text style={[styles.answer, trueAnswer ? styles.correctAnswer : styles.wrongAnswer]}>
              {selectedAnswer}
            </Text>
            {trueAnswer ?
          ...
          <View style={[styles.answerContainer, styles.correctAswerContainer]}>
            <Text style={[styles.answer, styles.correctAnswer]}>{correctAnswer}</Text>
            <Icon name="checkmark-outline" color={Colors.white} size={32} />
          </View>
        </View>
